...
struct Array2D
{
	...
	T& limitGet(int i) { return operator()(constrain(i, 0, area)); }
	T const& limitGet(int i) const { return operator()(constrain(i, 0, area)); }

	T& limitGet(Vec2i const& v) { return operator()(ci_area.closestPoint(v)); }
	T const& limitGet(Vec2i const& v) const { return operator()(ci_area.closestPoint(v)); }
	
	T& limitGet(int x, int y) { return limitGet(Vec2i(x, y)); }
	T const& limitGet(int x, int y) const { return limitGet(Vec2i(x, y)); }
...
private:
	...
	T* Init(int w, int h) {
		...
		//data = (T*)fftw_malloc(w * h * sizeof(T));
		data = new T[w * h];
		area = w * h;
		ci_area = Area(0, 0, w-1, h-1);
		this->w = w;
		this->h = h;
		return data;
	}
